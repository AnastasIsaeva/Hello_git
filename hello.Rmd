...
```{r}
shannon_index <- diversity(frequency_matrix, index = "shannon")

shannon_df <- data.frame(
  Year = as.numeric(rownames(frequency_matrix)),
  Shannon = shannon_index
)

ggplot(shannon_df, aes(x = Year, y = Shannon)) +
  geom_line() +
  geom_point() +
  labs(title = "Индекс Шеннона по количеству встречаемости артистов",
       x = "Год", y = "Индекс Шеннона") +
  theme_minimal()
```
